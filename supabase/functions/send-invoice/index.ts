...
function buildInvoiceEmail(d: {
  ...
  // trim defense.
  // Hard guard: only render the CTA when bank_link_url is present AND non-empty.
  // Current caller contract (contract-sign unified branch + regenerate-bank-link)
  // only triggers this with a valid Stripe Checkout URL, but an empty/null value
  // would otherwise render a CTA heading + button with an empty href — worse
  // UX than skipping the block entirely. Defensive against future contract drift.
  const hasBankLink = typeof d.bankLinkUrl === 'string' && d.bankLinkUrl.length > 0
  const ctaBlock = hasBankLink
    ? `<tr><td colspan="2" style="padding:24px 20px 4px;background:rgba(184,132,90,0.04);border-top:1px solid rgba(18,16,13,0.08)"><div style="font-family:'Barlow Condensed',Arial,sans-serif;font-size:11px;letter-spacing:2px;color:${BRAND_ACCENT};text-transform:uppercase;margin-bottom:4px">Action required</div><div style="font-family:'Cormorant Garamond',Georgia,serif;font-size:22px;font-weight:400;color:${BRAND_INK};line-height:1.2">Link your bank to pay</div></td></tr><tr><td colspan="2" style="padding:10px 20px 12px;background:rgba(184,132,90,0.04);font-size:14px;line-height:1.55;color:${BRAND_INK}">Click the button below to securely link your bank account via Stripe. Once linked, payment will be initiated on <strong>${esc(fmtDate(d.dueDate))}</strong>. Takes about 60 seconds.</td></tr><tr><td colspan="2" style="padding:4px 20px 16px;background:rgba(184,132,90,0.04)"><div style="font-family:'Barlow Condensed',Arial,sans-serif;font-size:11px;letter-spacing:2px;color:${BRAND_ACCENT};text-transform:uppercase;margin:0 0 8px">What to expect</div><ol style="margin:0;padding:0 0 0 22px;font-size:14px;line-height:1.5;color:${BRAND_INK};list-style:decimal outside"><li style="margin-bottom:4px">Click &quot;LINK YOUR BANK&quot; below</li><li style="margin-bottom:4px">Choose your bank and log in when Stripe prompts you</li><li style="margin-bottom:4px">Select your checking account</li><li style="margin-bottom:4px">Click the blue &quot;Back to Laviolette LLC&quot; button at the bottom</li><li style="margin-bottom:4px"><span style="font-weight:600">Click &quot;Set Up Bank Account&quot;</span> &mdash; <span style="color:${BRAND_ACCENT}">this is the final step and the one people most often miss</span></li><li style="margin-bottom:0">You'll land on a Laviolette success page &mdash; that means it worked</li></ol></td></tr><tr><td colspan="2" style="padding:0 20px 20px;background:rgba(184,132,90,0.04);text-align:center"><a href="${esc(d.bankLinkUrl)}" style="display:inline-block;padding:12px 32px;background:${BRAND_ACCENT};color:#12100D;text-decoration:none;border-radius:4px;font-family:'Barlow Condensed',Arial,sans-serif;font-size:13px;font-weight:700;letter-spacing:2px;text-transform:uppercase">Link your bank</a></td></tr><tr><td colspan="2" style="padding:0 20px 20px;background:rgba(184,132,90,0.04);font-size:11px;color:rgba(18,16,13,0.55);line-height:1.55;text-align:center">This link is valid for 24 hours. If it expires, reply to this email and we'll send a fresh one.</td></tr>`
    : ''
